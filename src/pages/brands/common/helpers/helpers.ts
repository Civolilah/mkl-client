...
export const validateBrand = async (brand: Brand) => {
  const validationSchema = Yup.object().shape({
    name: Yup.string()
      .required('brand_name_required')
      .max(200, 'brand_name_max_length'),
  });

  try {
    await validationSchema.validate(brand, { abortEarly: false });
