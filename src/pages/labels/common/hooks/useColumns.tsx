...
import { Label, LabelCategory, User } from '@interfaces/index';
...
const useColumns = (props: Props) => {
  ...
  const columns: TableColumnsType<Label> = [
    {
      title: t('name'),
      dataIndex: 'name',
      ...
      onCell: (record) => ({
        onClick: () =>
          navigate(route('/labels/:id/edit', { id: record.id as string })),
      }),
    },
    {
      title: t('label_category'),
      dataIndex: 'label_category',
      render: (value: LabelCategory | null) => {
        if (!value) {
          return <></>;
        }
        ...
        return (
          <Box className="min-w-56">
            <Link
              to={route('/label_categories/:id/edit', {
